...
import {
  useFlush,
  useMakeShitstrapPayment,
} from '../../../../../hooks/contracts/Shitstrap'
...
export const ShitstrapPaymentCard = ({
  ...
  const freshShitTokenQuery = useQueryLoadingDataWithError(
    tokenQueries.info(queryClient, {
      ...
      denomOrAddress: shitstrapInfo
        ? shitstrapInfo.shit.denomOrAddress
        : fallbackInfo.shit.denomOrAddress,
    })
  )

  const tokenToShit =
    freshShitTokenQuery.loading || freshShitTokenQuery.errored
      ? undefined
      : freshShitTokenQuery.data

  // return the asset that
  const eligibleAsset =
    watchShitToken && shitstrapInfo
  ...
  const selectedMicroBalance = watchShitToken
    ? (watchShitToken?.balance ?? 0)
    : 0
  ...
  const convertedShitRate = eligibleAsset
  ...
  const makeShitstrapPayment = useMakeShitstrapPayment({
    contractAddress: shitstrapInfo
      ? shitstrapInfo.shitstrapContractAddr
      : fallbackInfo.shitstrapContractAddr,
    ...
    <>
      <div className="rounded-lg bg-background-tertiary">
        ...
        <div className="flex flex-col gap-3 border-t border-border-secondary py-4 px-6">
          <p className="link-text  text-lg">
            {' '}
            {shitstrapInfo && shitstrapInfo.title}
          </p>
          <div className="flex flex-col gap-3 border-t border-border-secondary py-4 px-6">
            <p className="link-text">
              {' '}
              {shitstrapInfo && shitstrapInfo.description}
            </p>
          </div>
        </div>
            ...
            {shitstrapInfo &&
              ...
              shitstrapInfo.possibleShit.map((asset, index) => (
                ...
                <TokenAmountDisplay
                  ...
                  hideSymbol={false}
                  prefix="For every: "
                  suffix={`, Receive: ${HugeDecimal.from(asset.shit_rate ?? 1)
                    .div(HugeDecimal.from(10).pow(18))
                    .toNumber()} $${tokenToShit?.symbol} `}
                  symbol={asset.symbol}
                />
                // </div>
              ))

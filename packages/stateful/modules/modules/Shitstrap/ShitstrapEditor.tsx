...
import { AccountType, ActionKey, ModuleEditorProps } from '@dao-dao/types'
...
export const ShitStrapEditor = (
  props: ModuleEditorProps<ShitstrapPaymentModuleData>
) => {
  ...
  const { setError, clearErrors, watch } =
    useFormContext<ShitstrapPaymentModuleData>()
...
type ShitStrapChainProps = ModuleEditorProps<ShitstrapPaymentModuleData> & {
  /**
   * Chain ID.
   */
  chainId: string
...
const ShitStrapChain = ({
  ...
  const { watch, setValue } = useFormContext<ShitstrapPaymentModuleData>()

...
import { Add, Check, Close, WarningRounded } from '@mui/icons-material'
...
export const HandleDnasKeysRenderer: ActionComponent<ManageDnasActionData> = ({ fieldNamePrefix, isCreating, options }) => {
    ...
    const { unregisterDnasKey, addDnasToDao, dnas, profile, refreshProfile } = useDnas()
    const {
        control,
        register,
        setValue,
    ...
    useEffect(() => {
        if (actionType === ActionContextType.Dao && entityAddress) {
            setTimeout(() => {
                setValue((fieldNamePrefix + 'dnas.daoAddr') as 'dnas.daoAddr', entityAddress);
            }, 0)
        }
    }, [])

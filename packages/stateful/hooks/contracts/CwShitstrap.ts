...
import { ShitStrapClient as ExecuteClient } from '@dao-dao/state/contracts/ShitStrap'
import {
  ExecuteClientParams,
  executeClient,
} from '@dao-dao/state/recoil/selectors/contracts/CwShitstrap'
...
const wrapExecuteHook =
  ...
  (params: Omit<ExecuteClientParams, 'chainId'>) => {
    ...
    const { chainId: chainId } = useChain()
...
export const useMakeShitstrapPayment = wrapExecuteHook('shitStrap')
export const useFlush = wrapExecuteHook('flush')
export const useDelegate = wrapExecuteHook('refundShitter')

...
export const fetchInfusionById = async (
  ...
  const infusedCollectionInfo = infusion[0].infused_collection.addr
    ? await Promise.all([
      queryClient.fetchQuery(
        cw721BaseQueries.collectionInfo({
          chainId,
          contractAddress: infusion[0].infused_collection.addr,
        })
      ),
      queryClient.fetchQuery(
  ...
  const eligibleCollections = infusion[0].collections.map(async (cols) => {
    ...
    const colInfo = await Promise.all([
      ...
      queryClient.fetchQuery(
        cw721BaseQueries.contractInfo({ chainId, contractAddress: cols.addr })
      ),
    ])
    ...
    return {
      ...
      payment_substitute: paymentSub
        ? payment_substitute
        : cols.payment_substitute,
  ...
  const queryDataForInfusedCol = infusedCollectionInfo
    ? await Promise.all(infusedCollectionInfo)
    : null
  infusedCollection.image = queryDataForInfusedCol
    ? queryDataForInfusedCol[0].image
    : infusedCollection.image
